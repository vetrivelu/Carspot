...
class ExpiredAds extends Component<Props> {
  ...
  _onSwipeUp = async () => {
    ...
    setTimeout(async () => {
      ...
      orderStore.profile.data.expire_add.ads = [...this.defaultData];
      orderStore.profile.data.expire_add.pagination = this.paginationDefault;
      this.setState({ listData: this.defaultData, swipeUp: false, reCaller: false, refreshing: false, reRender: !this.state.reRender });
      if (orderStore.profile.data.expire_add.ads.length === 0)
        this.setState({ noDataVisivility: true, noDataMessage: orderStore.profile.data.expire_add.msg });
  ...
  componentWillMount() {
    let { orderStore } = Store;
    this.defaultData = [...orderStore.profile.data.expire_add.ads];
    this.paginationDefault = orderStore.profile.data.expire_add.pagination;
    this.setState({ listData: orderStore.profile.data.expire_add.ads });
    if (orderStore.profile.data.expire_add.ads.length === 0)
      this.setState({ noDataVisivility: true, noDataMessage: orderStore.profile.data.expire_add.msg });
  ...
  deleteItem = async (item, index) => {
    ...
    let response = await Api.post('ad/expired/remove', params);
    if (response.success === true) {

      let { orderStore } = Store;
      let data = [...orderStore.profile.data.expire_add.ads];
      data.splice(index, 1);
      orderStore.profile.data.expire_add.ads = data;
      this.setState({ listData: orderStore.profile.data.expire_add.ads });
  ...
  loadMore = async (nextPage) => {
    ...
    let response = await Api.post('ad/expired', params);
    if (response.success === true) {
      orderStore.profile.data.expire_add.pagination = response.data.pagination;
      orderStore.profile.data.expire_add.ads = [...orderStore.profile.data.expire_add.ads, ...response.data.ads];
      this.setState({ listData: orderStore.profile.data.expire_add.ads });
  ...
  recallService = async (nextPage) => {
    ...
    if (response.success === true) {
      ...
      orderStore.profile.data.expire_add.pagination = response.data.pagination;
      orderStore.profile.data.expire_add.ads = response.data.ads;
      this.setState({ listData: orderStore.profile.data.expire_add.ads, reCaller: false });
  ...
  _onRefresh = () => {
    ...
    let { orderStore } = Store;
    let pagination = orderStore.profile.data.expire_add.pagination;
  ...
  _onRefreshControl = async () => {
    ...
    if (orderStore.profile.success == true) {
      this.defaultData = [...orderStore.profile.data.expire_add.ads];
      this.paginationDefault = orderStore.profile.data.expire_add.pagination;
      this.setState({ listData: orderStore.profile.data.expire_add.ads,refreshing:false });
      if (orderStore.profile.data.expire_add.ads.length === 0)
        this.setState({ noDataVisivility: true, noDataMessage: orderStore.profile.data.expire_add.msg,refreshing:false });
        ...
        <ScrollView
          ...
          <View style={styles.container}>
            {this.state.listData.length != 0 ? <View style={styles.headingContainer}>
              <Text style={styles.bodyHeadingBlack}>{orderStore.profile.extra_text.expired_title + ' ' + orderStore.profile.extra_text.ads_title}</Text>
            </View> : null}
